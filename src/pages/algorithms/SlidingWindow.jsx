...
function SlidingWindow({ onBack, onPrevious, onNext, previousName, nextName, currentSubcategory, previousSubcategory, nextSubcategory, onPreviousSubcategory, onNextSubcategory, breadcrumb, breadcrumbStack, onBreadcrumbClick, pushBreadcrumb, breadcrumbColors, problemLimit }) {
  ...
  const getCompletionStats = () => {
    const completed = displayQuestions.filter(q => isProblemCompleted(`Sliding Window-${q.id}`)).length
  ...
  const selectQuestion = (question) => {
    ...
    const savedDrawing = localStorage.getItem(`drawing-SlidingWindow-${question.id}`)
  ...
  if (selectedQuestion) {
    ...
    const problemBreadcrumbStack = breadcrumbStack
      ? [...breadcrumbStack.slice(0, -1), { name: 'Sliding Window', page: null }, { name: selectedQuestion.title, page: null }]
      : null

    // Fallback to legacy format
    const problemBreadcrumb = {
      ...breadcrumb,
      category: { name: 'Sliding Window', onClick: () => setSelectedQuestion(null) },
      ...
      <div style={{ padding: '2rem', maxWidth: '1800px', margin: '0 auto', background: 'linear-gradient(to bottom right, #111827, #1e3a5f, #111827)', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          ...
          <div style={{ backgroundColor: '#1f2937', padding: '2rem', borderRadius: '12px', border: '2px solid #374151', maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
              ...
              <BookmarkButton problemId={`SlidingWindow-${selectedQuestion.id}`} problemData={{ title: selectedQuestion.title, difficulty: selectedQuestion.difficulty, category: 'SlidingWindow' }} />
            </div>
          ...
          <div style={{ backgroundColor: '#1f2937', padding: '2rem', borderRadius: '12px', border: '2px solid #374151', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
            ...
            {output && (
              <div style={{ marginTop: '1rem' }}>
                ...
                <pre style={{ backgroundColor: '#374151', padding: '1rem', borderRadius: '8px', border: '1px solid #374151', overflow: 'auto', fontSize: '0.875rem', maxHeight: '150px', color: '#9ca3af' }}>{output}</pre>
              </div>
            )}
          </div>
        </div>
        ...
        <DrawingCanvas
          ...
          onClose={() => {
            ...
            const savedDrawing = localStorage.getItem(`drawing-SlidingWindow-${selectedQuestion.id}`)
            setCurrentDrawing(savedDrawing)
          }}
          problemId={`SlidingWindow-${selectedQuestion.id}`}
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', background: 'linear-gradient(to bottom right, #111827, #1e3a5f, #111827)', minHeight: '100vh' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: '800', background: 'linear-gradient(to right, #93c5fd, #60a5fa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', marginBottom: '0.5rem' }}>Sliding Window</h1>
        <p style={{ fontSize: '1.2rem', color: '#9ca3af' }}>Master the sliding window technique for substring and subarray problems</p>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div onClick={(e) => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <BookmarkButton size="small" problemId={`SlidingWindow-${question.id}`} problemData={{ title: question.title, difficulty: question.difficulty, category: 'SlidingWindow' }} />
