...
def twoSum(self, numbers: List[int], target: int) -> List[int]:
  ...
  // When problemLimit is set, show only the first N most important problems
  const displayQuestions = problemLimit ? questions.slice(0, problemLimit) : questions

  // Calculate completion status
  const getCompletionStats = () => {
    const completed = displayQuestions.filter(q => isProblemCompleted(`Arrays-${q.id}`)).length
          ...
          <div style={{ backgroundColor: '#1f2937', padding: '2rem', borderRadius: '12px', border: '2px solid #374151', maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            {selectedQuestion.explanation && (
              <div style={{ marginTop: '1.5rem' }}>
                ...
                <p style={{ fontSize: '0.95rem', color: '#9ca3af', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>{selectedQuestion.explanation}</p>
              </div>
            )}
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', background: 'linear-gradient(to bottom right, #111827, #1e3a5f, #111827)', minHeight: '100vh' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        ...
        <p style={{ fontSize: '1.2rem', color: '#9ca3af' }}>Master arrays problems</p>
