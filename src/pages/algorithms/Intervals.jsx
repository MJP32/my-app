...
function Intervals({ onBack, breadcrumb, breadcrumbStack, onBreadcrumbClick, pushBreadcrumb, breadcrumbColors, problemLimit }) {
  ...
  const getCompletionStats = () => {
    const completed = displayQuestions.filter(q => isProblemCompleted(`Intervals-${q.id}`)).length
  ...
  if (selectedQuestion) {
    ...
    const problemBreadcrumb = {
      ...breadcrumb,
      category: { name: 'Intervals', onClick: () => setSelectedQuestion(null) },
      topic: selectedQuestion.title
    }

    const problemBreadcrumbStack = breadcrumbStack
      ? [...breadcrumbStack.slice(0, -1), { name: 'Intervals', page: null }, { name: selectedQuestion.title, page: null }]
      : null
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', background: 'linear-gradient(to bottom right, #111827, #1e3a5f, #111827)', minHeight: '100vh' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: '800', background: 'linear-gradient(to right, #93c5fd, #60a5fa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', marginBottom: '0.5rem', color: 'white' }}>Intervals</h1>
        <p style={{ fontSize: '1.2rem', color: '#9ca3af' }}>Master interval-based problems: merging, overlapping, and scheduling</p>
