...
function FamousAlgorithms({ onBack, onPrevious, onNext, previousName, nextName, currentSubcategory, previousSubcategory, nextSubcategory, onPreviousSubcategory, onNextSubcategory, breadcrumb, breadcrumbStack, onBreadcrumbClick, pushBreadcrumb, breadcrumbColors, problemLimit }) {
...
def majority_element_bit(self, nums: List[int]) -> int:
  ...
  const getCompletionStats = () => {
    const completed = displayQuestions.filter(q => isProblemCompleted(`Famous Algorithms-${q.id}`)).length
  ...
  if (selectedQuestion) {
    ...
    const problemBreadcrumbStack = breadcrumbStack
      ? [...breadcrumbStack.slice(0, -1), { name: 'Famous Algorithms', page: null }, { name: selectedQuestion.title, page: null }]
      : null

    // Fallback to legacy format
    const problemBreadcrumb = {
      ...breadcrumb,
      category: { name: 'Famous Algorithms', onClick: () => setSelectedQuestion(null) },
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          ...
          <div style={{ backgroundColor: '#1f2937', padding: '2rem', borderRadius: '12px', border: '2px solid #374151', maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
              ...
              <BookmarkButton problemId={`FamousAlgorithms-${selectedQuestion.id}`} problemData={{ title: selectedQuestion.title, difficulty: selectedQuestion.difficulty, category: 'FamousAlgorithms' }} />
            </div>
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', background: 'linear-gradient(to bottom right, #111827, #1e3a5f, #111827)', minHeight: '100vh' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: '800', color: 'white', marginBottom: '0.5rem' }}>Famous Algorithms</h1>
        <p style={{ fontSize: '1.2rem', color: '#9ca3af' }}>Master famous algorithms problems</p>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
                      ...
                      <div onClick={(e) => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <BookmarkButton size="small" problemId={`FamousAlgorithms-${question.id}`} problemData={{ title: question.title, difficulty: question.difficulty, category: 'FamousAlgorithms' }} />
