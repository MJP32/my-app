#!/usr/bin/env python3
"""
Convert algorithm category files to match SlidingWindow.jsx format.
This script updates 22 algorithm files to use consistent structure, imports, and UI patterns.
"""
...
# Category configurations (emoji, title, problemId prefix)
CATEGORIES = {
...
def extract_questions_array(content):
    """Extract the questions array from file content"""
    # Find the questions array - it starts with 'const questions = [' or just 'const questions ='
    ...
    for i in range(match.end(), len(content)):
        ...
        if char == '[':
            ...
            bracket_count -= 1
            if in_array and bracket_count == -1:
                end_pos = i + 1
                break
...
def ensure_question_format(questions_str, component_name):
    """Ensure each question has required fields"""
    # This is a placeholder - in reality, we'll preserve existing questions as-is
    # and just ensure they have the timeComplexity and spaceComplexity if missing
    return questions_str

def generate_component(filename, emoji, title, problem_id_prefix, questions_array):
...
function {component_name}({{ onBack, onPrevious, onNext, previousName, nextName, currentSubcategory, previousSubcategory, nextSubcategory, onPreviousSubcategory, onNextSubcategory }}) {{
  ...
  const getCompletionStats = () => {{
    const completed = questions.filter(q => isProblemCompleted(`{problem_id_prefix}-${{q.id}}`)).length
        ...
        <div style={{{{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}}}>
          ...
          <div style={{{{ backgroundColor: 'white', padding: '2rem', borderRadius: '12px', border: '2px solid #e5e7eb', maxHeight: '85vh', overflowY: 'auto' }}}}>
            ...
            <div style={{{{ marginBottom: '1.5rem' }}}}>
              <CompletionCheckbox problemId={{`{problem_id_prefix}-${{selectedQuestion.id}}`}} />
            </div>
  ...
  return (
    <div style={{{{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', backgroundColor: '#f0f9ff', minHeight: '100vh' }}}}>
      ...
      <div style={{{{ textAlign: 'center', marginBottom: '3rem' }}}}>
        <h1 style={{{{ fontSize: '2.5rem', fontWeight: '800', color: '#1f2937', marginBottom: '0.5rem' }}}}>{emoji} {title}</h1>
        <p style={{{{ fontSize: '1.2rem', color: '#6b7280' }}}}>Master {title.lower()} problems</p>
                ...
                {{difficultyQuestions.map((question) => (
                  <div key={{question.id}} onClick={{() => selectQuestion(question)}} style={{{{ backgroundColor: 'white', padding: '1.5rem', borderRadius: '12px', border: '2px solid #e5e7eb', cursor: 'pointer', transition: 'all 0.2s' }}}} onMouseEnter={{(e) => {{ e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.boxShadow = '0 8px 16px rgba(0,0,0,0.1)' }}}} onMouseLeave={{(e) => {{ e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none' }}}}>
                    <div style={{{{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '0.75rem' }}}}>
                      ...
                      {{isProblemCompleted(`{problem_id_prefix}-${{question.id}}`) && <span style={{{{ fontSize: '1.25rem' }}}}>✅</span>}}
                    </div>
...
def process_file(filepath):
    ...
    if filename not in CATEGORIES:
        print(f"Skipping {filename} - not in category list")
        return False

    emoji, title, problem_id_prefix = CATEGORIES[filename]
    ...
    try:
        ...
        questions_array = extract_questions_array(content)

        if not questions_array:
            print(f"  WARNING: Could not extract questions array from {filename}")
            return False

        # Ensure question format
        questions_array = ensure_question_format(questions_array, filename.replace('.jsx', ''))

        # Generate new component
        new_content = generate_component(filename, emoji, title, problem_id_prefix, questions_array)
...
def main():
    ...
    algorithms_dir = Path(__file__).parent / 'src' / 'pages' / 'algorithms'

    if not algorithms_dir.exists():
        print(f"Error: Directory not found: {algorithms_dir}")
        return

    print(f"Processing algorithm files in: {algorithms_dir}\n")
